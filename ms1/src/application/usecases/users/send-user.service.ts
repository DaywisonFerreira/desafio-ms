...
import { CreateKafkaMessage } from 'src/infra/utils/helpers';
...
export class SendUserService {
  ...
  async execute(userDto: CreateUserDto): Promise<void> {
    try {
      const payload = CreateKafkaMessage(KAFKA_TOPIC_CREATE_NEW_USER, userDto);
      await this.messageProvider.sendMessage(payload);
      this.logger.log(
        `Successfully published new user with e-mail: ${userDto.email}`,
      );
    } catch (error) {
      this.logger.error(
        `Error while publishing new user with e-mail: ${userDto.email}`,
        error,
      );
